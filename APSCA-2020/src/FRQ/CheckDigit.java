   ...
   {   
   	//WRITE CODE FOR THIS METHOD
		...
		while(num2>0)
		{
			...
			if (num1size -1 = num2size)
			{
		
		
				if(dig /10 == numWithCheckDigit)
				{
					return true;
				}
		}
...
add a private counter that can be used in the method and it wont be overwriten if the method is called again.
...
new variable to hold the number of times it has been called incorrectly.
there will be a new if statement to go with incorrect method calling
